...
def avoid_obstacle(cur, avoidances, matrix, obj, old_path, old_avoidances, i, vis, new_end_node, big, thres):
    sphere2 = obj[1]
...
def init_vis(lines, best_path, geom):
    ...
    mid = (len(best_path) // 2) + 800

    obstacle_positions = [best_path[len(best_path) - (len(best_path)//2) - 700], best_path[len(best_path) - (len(best_path)//3)], best_path[mid]]
    ...
    while True:
        for i in range(0, len(best_path)-3, 25):
            ...
            if 0.1 <= val1 < 420.0 and ok:
                ...
                # ok = False
                avoid_obstacle(best_path[i], avoidances, matrix, obj, old_path, old_avoidances, i, vis, end_node, False, 3)
            elif 0.1 <= val < 420.0 and ok2:
                sphere2.paint_uniform_color([1, 0.706, 0])
                # need a function to do back conversions to approximate location of obj
                # print(old_path[i])
                # matrix[round(old_path[i][2])][round(old_path[i][1])][round(old_path[i][0])] = 255
