...
class astar_3d_algo:
    ...
    def get_neighbors(self, current, grid):
        ...
        # Right
        if current[0] + 1 < self.map_size[0] and grid[current[2], current[1], current[0] + 1] == 0:
            neighbors.add((current[0] + 1, current[1], current[2]))
        # Up
        if current[1] - 1 >= 0 and grid[current[2], current[1] - 1, current[0]] == 0:
            neighbors.add((current[0], current[1] - 1, current[2]))
        # Left
        if current[0] - 1 >= 0 and grid[current[2], current[1], current[0] - 1] == 0:
            neighbors.add((current[0] - 1, current[1], current[2]))
        # Down
        if current[1] + 1 < self.map_size[1] and grid[current[2], current[1] + 1, current[0]] == 0:
            neighbors.add((current[0], current[1] + 1, current[2]))
        # ↗
        if current[0] + 1 < self.map_size[0] and current[1] - 1 >= 0 and \
                grid[current[2], current[1] - 1, current[0] + 1] == 0:
        ...
        if current[0] - 1 >= 0 and current[1] + 1 < self.map_size[1] and \
            ...
            neighbors.add((current[0] - 1, current[1] + 1, current[2]))
        # Z-Up
        if current[2] + 1 < self.map_size[2] and grid[current[2] + 1, current[1], current[0]] == 0:
            neighbors.add((current[0], current[1], current[2] + 1))
        # Z-Down
        if current[2] - 1 >= 0 and grid[current[2] - 1, current[1], current[0]] == 0:
            neighbors.add((current[0], current[1], current[2] - 1))
        
        # Z-Up Left
        if current[2] + 1 < self.map_size[2] and current[0] - 1 >= 0 and \
                grid[current[2] + 1, current[1], current[0] - 1] == 0:
        ...
        if current[2] + 1 < self.map_size[2] and current[1] - 1 >= 0 and \
                grid[current[2] + 1, current[1] - 1, current[0]] == 0:
        ...
        # Z-Up ↗
        if current[2] + 1 < self.map_size[2] and current[0] + 1 < self.map_size[0] and \
                current[1] - 1 >= 0 and grid[current[2] + 1, current[1] - 1, current[0] + 1] == 0:
            neighbors.add((current[0] + 1, current[1] - 1, current[2] + 1))
        # Z-Up ↘
        if current[2] + 1 < self.map_size[2] and current[0] + 1 < self.map_size[0] and \
                current[1] + 1 < self.map_size[1] and grid[current[2] + 1, current[1] + 1, current[0] + 1] == 0:
            neighbors.add((current[0] + 1, current[1] + 1, current[2] + 1))
        # Z-Up ↙
        if current[2] + 1 < self.map_size[2] and current[0] - 1 >= 0 and \
                current[1] + 1 < self.map_size[1] and grid[current[2] + 1, current[1] + 1, current[0] - 1] == 0:
    ...
    def find_path(self, matrix):
        ...
        while not frontier.empty():
            ...
            for next in neighbors:
                ...
                # print(new_cost)
                new_cost = cost_value[current] + (func ** 0.5) + self.cost_map[next]
