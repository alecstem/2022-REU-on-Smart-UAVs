'''
This is the code that has the adapted A* with all the different elements tested.
'''
import heapq
import time
import numpy as np
...
class astar_3d_algo:
    ...
    def get_distance(self, current, end_point):
        ...
        # distance = (abs(current[0] - end_point[0])) + (abs(current[1] - end_point[1])) + (abs(current[2] - end_point[2]))
        return distance
        ...
        while not frontier.empty():
            ...
            for next in neighbors:
                ...
                try:
                    ...
                    # new_cost = cost_value[current] + (func ** 0.5) + self.cost_map[(next[2], next[1], next[0])] + d[(next[0], next[1], next[2])]
                    # new_cost = cost_value[current] + self.cost_map[(next[2], next[1], next[0])]
                    new_cost = cost_value[current] + self.cost_map[(next[2], next[1], next[0])] + d[(next[0], next[1], next[2])]
